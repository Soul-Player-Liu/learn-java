...
- 保留自己的页面结构和平台反馈。
- 选择一个主 UI 库承载基础组件，再用项目级 design tokens 统一字号、间距、色彩和触控尺寸。
- 在本仓库中，移动端以 Wot Design Uni 为主 UI 库，uni-ui 作为官方补充组件库。
...
大型项目建议采用短入口和结构化 `docs/`，但具体分层、链接和联动更新规则由 [documentation.md](documentation.md) 负责。

Monorepo 标准只要求文档能回答：当前系统怎么工作、边界如何协作、标准是什么、如何验证标准没有漂移。它不要求每个项目机械复制同一套文件名。
